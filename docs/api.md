...
## High-Risk Correlated Attack Target Scenario

The seeded database and simulator adapters produce the coordinated scenario centered on `PAYMENT-API-01`:

```text
PAYMENT-API-01 (Payment API)
  ├── Asset: Criticality = CRITICAL | Internet Exposed = TRUE | Value = $48,000,000 | AWS
  ├── Vulnerability: CVE-2024-21762 (CVSS 9.8, Known Exploited = TRUE, Priority = CRITICAL_EXPLOITED_EXPOSED)
  ├── IAM: USR-001 (admin.singh) | Role = database_admin | Privileged = TRUE | MFA = FALSE | Failed Logins = 17
  ├── SIEM: BRUTE_FORCE (T1110) from Attacker IP 185.10.20.30 (Critical Alert)
  ├── EDR: SUSPICIOUS_PROCESS (powershell.exe, Indicator = credential_dumping)
  └── CSPM: OPEN_SECURITY_GROUP (Critical Misconfiguration: 0.0.0.0/0 on sensitive ports)
```
