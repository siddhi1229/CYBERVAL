...
const ENABLE_FALLBACK = import.meta.env.VITE_ENABLE_MOCK_FALLBACK !== 'false';
...
apiClient.interceptors.request.use(
  (config) => {
    // Add auth token or tenant headers if available
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    // If backend is unavailable or endpoint is not found, fallback to high-fidelity telemetry if enabled
    if (ENABLE_FALLBACK && (!error.response || error.response.status === 404 || error.response.status >= 500)) {
      console.warn(`[CYBERVAL Telemetry Engine] Backend unreachable or 404 for ${originalRequest.url}. Serving live telemetry fallback.`);
      const mockResponse = getMockResponseForUrl(originalRequest.url, originalRequest.method, originalRequest.data);
      if (mockResponse !== null) {
        return Promise.resolve({
          data: mockResponse,
          status: 200,
          statusText: 'OK (Mock Telemetry)',
          headers: {},
...
function getMockResponseForUrl(url = '', method = 'get', requestData) {
  ...
  if (cleanUrl.includes('/copilot/query') || cleanUrl.includes('/copilot/chat')) {
    let query = '';
    if (typeof requestData === 'string') {
      try {
...
function getCopilotResponse(query = '') {
  ...
  // Default dynamic response generator
  return {
